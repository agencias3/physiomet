            ...
            <div class="panel-body">
                ...
                <div class="row">
                    <div class="col-md-12 text-right">
                        <a href="{{ route('admin.partner.export').$config['route']['queryString'] }}" class="mb-xs mt-xs mr-xs btn btn-warning"><i class="fa fa-file-excel-o"></i> Exportar</a><br /><br />
                    </div>
                </div>
                @include('admin.partner._form_filter')
                ...
                <table class="table table-no-more table-bordered table-striped mb-0">
                    <thead>
                        <tr>
                            <th>Razão Social ({{ $dados->total() }})</th>
                            <th>E-mail</th>
                            <th>Telefone</th>
                    ...
                    @foreach($dados as $row)
                        <tr>
                            <td data-title="Nome">{{ $row->social_reason }}</td>
                            ...
                            <td data-title="Ação" class="actions text-center">
                                <a href="{{ route('admin.partner.show', ['id' => $row->id]) }}" class="btn btn-default white-hover" title="Visualizar"><i class="fa el-icon-search"></i></a>
                                <a href="#modalDestroy" data-route="{{ route('admin.partner.destroy', ['id' => $row->id]) }}" class="excluir remove-row btn btn-danger white" title="Excluir">
