        ...
        <section class="panel media-gallery">
            ...
            <div class="panel-body">
                ...
                <div class="row mg-files" data-sort-destination data-sort-id="media-gallery">
                    <ul class="sortable col-sm-12" data-route="{{ route('admin.blog.post.gallery.order', ['id' => $id]) }}">
                        @foreach($dados as $row)
                        <li class="lisort col-sm-3 div_{{ $row->id }}" id="item_{{ $row->id }}">
                            <div class="isotope-item document">
                                <div class="thumbnail">
                                    <div class="thumb-preview">
                                        <a class="thumb-image" href="{{ asset('uploads/post/'.$row->image) }}" style="height: 221px; background: url('{{ asset('uploads/post/'.$row->image) }}') center center; -webkit-background-size: cover;background-size: cover;"></a>
                                        <div class="mg-thumb-options">
                                            ...
                                            <div class="mg-toolbar">
                                                <div class="mg-group pull-right">
                                                    <a href="#modalForm" data-id="{{ $row->id }}" data-route="{{ route('admin.blog.post.gallery.updateLabel', ['id' => $row->id]) }}" class="modal-form btnEditLabel">LEGENDA</a>
                                                    <a href="#modalDestroy" data-id="{{ $row->id }}" data-route="{{ route('admin.blog.post.gallery.destroy', ['id' => $row->id]) }}" class="modal-form excluir" data-id="<?php echo $id; ?>"><i class="fa fa-trash-o"></i> EXCLUIR</a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <h5 class="mg-title text-semibold text-right col-md-12">
                                        CAPA
                                        <input type="checkbox" data-route="{{ route('admin.blog.post.gallery.cover', ['id' => $row->id]) }}" <?php if($row->cover == 'y'){echo 'checked="checked"';} ?> class="cover" data-id="{{ $row->id }}" data-id-album="{{ $id }}" name="capa" />
                                    </h5>
