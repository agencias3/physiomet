...
use AgenciaS3\Validators\MachineImageValidator;
...
class MachineImageController extends Controller
{
    ...
    public function __construct(MachineImageRepository $repository,
                                MachineImageValidator $validator,
                                UtilObjeto $utilObjeto)
    {
        $this->repository = $repository;
        $this->validator = $validator;
        $this->utilObjeto = $utilObjeto;
        $this->path = 'uploads/machine/machine/';
    }
    ...
    public function index($id)
    {
        $config = $this->header();
        $config['galery'] = true;
        $routeUpload = route('admin.machine.machine.gallery.upload', ['id' => $id]);

        $dados = $this->repository->orderBy('order', 'asc')->findWhere(['machine_id' => $id]);

        return view('admin.machine.machine.gallery.index', compact('dados', 'id', 'config', 'routeUpload'));
    }

    public function header()
    {
        $config['title'] = "Máquinas > Galeria";
    ...
    public function upload(AdminRequest $request, $id)
    {
        ...
        $verifica = $this->repository->findWhere(['machine_id' => $id]);
        if ($verifica->toArray()) {
            $data['cover'] = 'n';
        } else {
            $data['cover'] = 'y';
    ...
    {
        return $this->repository->deleteWhere(['machine_id' => $id]);
    }
    ...
    public function cover(AdminRequest $request, $id)
    {
        ...
        $this->repository->scopeQuery(function ($query) use ($data) {
            $query->where([
                'machine_id' => $data['id_album'],
                'cover' => 'y'
            ])->update(['cover' => 'n']);
            return $query->where(['machine_id' => $data['id_album'], 'cover' => 'n']);
        })->all();
