...
class TypeImageController extends Controller
{
    ...
    public function __construct(TypeImageRepository $repository,
                                TypeImageValidator $validator,
    ...
    public function index($id)
    {
        $config = $this->header();
        $config['galery'] = true;
        $routeUpload = route('admin.type.gallery.upload', ['id' => $id]);

        $dados = $this->repository->orderBy('order', 'asc')->findWhere(['type_id' => $id]);

        return view('admin.type.gallery.index', compact('dados', 'id', 'config', 'routeUpload'));
    }

    public function header()
    {
        $config['title'] = "Tipo Fisioterapia > Galeria";
    ...
    public function upload(AdminRequest $request, $id)
    {
        ...
        $verifica = $this->repository->findWhere(['type_id' => $id]);
        if ($verifica->toArray()) {
            $data['cover'] = 'n';
        } else {
            $data['cover'] = 'y';
    ...
    {
        return $this->repository->deleteWhere(['type_id' => $id]);
    }
    ...
    public function cover(AdminRequest $request, $id)
    {
        ...
        $this->repository->scopeQuery(function ($query) use ($data) {
            $query->where([
                'type_id' => $data['id_album'],
                'cover' => 'y'
            ])->update(['cover' => 'n']);
            return $query->where(['type_id' => $data['id_album'], 'cover' => 'n']);
        })->all();
