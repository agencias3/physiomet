...
class ServiceImageController extends Controller
{
    ...
    public function __construct(ServiceImageRepository $repository,
                                ServiceImageValidator $validator,
    ...
    public function index($id)
    {
        $config = $this->header();
        $config['galery'] = true;
        $routeUpload = route('admin.service.gallery.upload', ['id' => $id]);

        $dados = $this->repository->orderBy('order', 'asc')->findWhere(['service_id' => $id]);

        return view('admin.service.gallery.index', compact('dados', 'id', 'config', 'routeUpload'));
    }

    public function header()
    {
        $config['title'] = "Serviços > Galeria";
    ...
    public function upload(AdminRequest $request, $id)
    {
        ...
        $verifica = $this->repository->findWhere(['service_id' => $id]);
        if ($verifica->toArray()) {
            $data['cover'] = 'n';
        } else {
            $data['cover'] = 'y';
    ...
    public function cover(AdminRequest $request, $id)
    {
        ...
        $this->repository->scopeQuery(function ($query) use ($data) {
            $query->where([
                'service_id' => $data['id_album'],
                'cover' => 'y'
            ])->update(['cover' => 'n']);
            return $query->where(['service_id' => $data['id_album'], 'cover' => 'n']);
        })->all();
