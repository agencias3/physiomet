...
namespace AgenciaS3\Http\Controllers\Admin\Enterprise;
...
class EnterpriseImageController extends Controller
{
    ...
    public function __construct(EnterpriseImageRepository $repository,
                                EnterpriseImageValidator $validator,
                                UtilObjeto $utilObjeto)
    {
        $this->repository = $repository;
        $this->validator = $validator;
        $this->utilObjeto = $utilObjeto;
        $this->path = 'uploads/enterprise/';
    }
    ...
    public function index($id)
    {
        $config = $this->header();
        $config['galery'] = true;
        $routeUpload = route('admin.enterprise.gallery.upload', ['id' => $id]);

        $dados = $this->repository->orderBy('order', 'asc')->findWhere(['enterprise_id' => $id]);

        return view('admin.enterprise.gallery.index', compact('dados', 'id', 'config', 'routeUpload'));
    }

    public function header()
    {
        $config['title'] = "Empreendimentos > Galeria";
        $config['activeMenu'] = 'enterprise';
        $config['activeMenuN2'] = 'enterprise';
    ...
    public function upload(AdminRequest $request, $id)
    {
        ...
        $verifica = $this->repository->findWhere(['enterprise_id' => $id]);
        if ($verifica->toArray()) {
            $data['cover'] = 'n';
        } else {
            $data['cover'] = 'y';
    ...
    {
        return $this->repository->deleteWhere(['enterprise_id' => $id]);
    }
    ...
    public function cover(AdminRequest $request, $id)
    {
        ...
        $this->repository->scopeQuery(function ($query) use ($data) {
            $query->where([
                'enterprise_id' => $data['id_album'],
                'cover' => 'y'
            ])->update(['cover' => 'n']);
            return $query->where(['enterprise_id' => $data['id_album'], 'cover' => 'n']);
        })->all();
